...
LinkList mixLinkList(LinkList A, LinkList B) {
    ...
    for (ListNode *pa = A->next; pa->next != NULL; pa = pa->next) {
        
        for (ListNode *pb = B->next; pb->next != NULL; pb = pb->next) {
            
            DataType data = pa->data;
            NSLog(@"Tag %d", data);
...
- (void)testLinkList_10 {
    ...
    LinkList A = creatLinkListA_10(10);
    printLinkList_10(A);
    
    LinkList B = creatLinkListB_10(15);
    printLinkList_10(B);
